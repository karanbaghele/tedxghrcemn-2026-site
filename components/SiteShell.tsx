...
function PartnersPage(){const partners=[{title:"Hosted by / Institutional Support"},{title:"Sponsors"},{title:"Knowledge Partners"},{title:"Community Partners",name:"GDG Nagpur",label:"Official Community Partner",image:"/partners/gdg-nagpur-card.jpeg"},{title:"Media Partners",name:"Triomotion Studios",label:"Official Media Partner",image:"/partners/triomotion-studios-card.jpeg"},{title:"Hospitality Partners"}];return <main id="main-content"><PageHero label="PARTNERS / COLLABORATE" title="Support an exchange of ideas" copy="Meet the confirmed partners supporting TEDxGHRCEMN 2026."/><section className="section partner-list">{partners.map((partner,i)=><div key={partner.title}><span>{String(i+1).padStart(2,"0")}</span><h2>{partner.title}</h2>{partner.image?<figure className="partner-logo-card"><img src={partner.image} alt={`${partner.name}, ${partner.label}`}/><figcaption><strong>{partner.name}</strong><small>{partner.label}</small></figcaption></figure>:<p>Partners to Be Announced</p>}</div>)}</section><section className="partner-cta"><p className="eyebrow">PARTNER WITH US</p><h2>Help create space for<br/><em>ideas and people.</em></h2><p>Partnership formats and approved enquiry details will be shared soon.</p><a className="button primary" href="/contact">Start an enquiry <ArrowRight size={17}/></a></section></main>}
